...
const std::unordered_map<Codec::ID, Codec::Info::PointerType> Registry::c_codec_id_map = []() -> std::unordered_map<Codec::ID, Codec::Info::PointerType> {
	std::unordered_map<Codec::ID, Codec::Info::PointerType> map;
	for (auto& codec_ptr : c_codec_registry) {
		map[codec_ptr->ID()] = codec_ptr;
	}
	return map;
}();
...
const std::unordered_map<std::string, Codec::Info::PointerType> Registry::c_codec_name_map = []() -> std::unordered_map<std::string, Codec::Info::PointerType> {
	...
	for (auto& codec_ptr : c_codec_registry) {
		map[StormByte::Util::String::ToLower(codec_ptr->Name())] = codec_ptr;
	}
	return map;
}();

Codec::Info::PointerType Registry::CodecInfo(const Codec::ID& codec) {
	const auto& it = c_codec_id_map.find(codec);

	return (it != c_codec_id_map.end()) ? it->second : nullptr;
}
...
const std::unordered_map<Container::ID, Container::Info::PointerType> Registry::c_container_id_map = []() -> std::unordered_map<Container::ID, Container::Info::PointerType> {
    std::unordered_map<Container::ID, Container::Info::PointerType> map;
    for (auto& container_ptr: Registry::c_container_registry) {
        map[container_ptr->ID()] = container_ptr;
    }
    return map;
}();
...
const std::unordered_map<std::string, Container::Info::PointerType> Registry::c_container_name_map = []() -> std::unordered_map<std::string, Container::Info::PointerType> {
    ...
    for (auto container_ptr: Registry::c_container_registry) {
        map[StormByte::Util::String::ToLower(container_ptr->Name())] = container_ptr;
    }
    return map;
}();

Container::Info::PointerType Registry::ContainerInfo(const Container::ID& container) {
    const auto& it = c_container_id_map.find(container);

	return (it != c_container_id_map.end()) ? it->second : nullptr;
}
