...
namespace StormByte::Multimedia::Pipeline::Filter::Audio {
	 ...
	 *
	 * @par Algorithm
	 * Graph is @c aresample=osr=&lt;rate&gt; only. Empty @a rate
	 * here is 48000. The swr backend inside `aresample` is the
	 * FFmpeg default; this leaf never sets @c resampler=soxr.
	 * Same input rate as the target is a no-op (no graph, no
	 * @ref Filter::FFmpeg::Save). Rate &lt;= 0 Fails. Layout and
	 * sample format are left to the graph. Hardware does not
	 * apply to audio.
	 *
	 * @par Mutation
	 ...
	 * from the handle. Empty sink with a successful Filter
	 * is EAGAIN: log wait and return. @ref Eof flushes.
	 *
