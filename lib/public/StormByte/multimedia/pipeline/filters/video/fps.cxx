...
#include <StormByte/multimedia/pipeline/filters/video/fps.hxx>
...
using StormByte::Multimedia::Pipeline::Filter::Video::Fps;
...
void Fps::Clean() noexcept {
	m_graph.reset();
}

void Fps::Setup() noexcept {
	Clean();
}
...
void Fps::Process(const Pipeline::Frame& frame) noexcept {
	...
	if (!src || src.Width() <= 0 || src.Height() <= 0) {
		Log(Level::Warning, "fps: frame has no picture");
		return;
	}
	if (src.Hardware()) {
		Fail("fps: hardware frame; decode to software first");
		return;
	}
	if (m_num == 0) {
		Fail("fps: rate numerator must be positive");
		return;
	}
	...
	if (!m_graph->Filter(src, out)) {
		Fail("fps: AVFilterGraph::Filter failed");
		return;
	}
	if (!out) {
		Log(Level::LowLevel, std::format(
			"fps wait {}x{} pts={}", src.Width(), src.Height(), src.Pts()));
		return;
	}
...
void Fps::Eof() noexcept {
	if (!m_graph)
		return;
	for (;;) {
		FFrame out;
		if (!m_graph->Flush(out)) {
			Fail("fps: AVFilterGraph::Flush failed");
			break;
		}
		if (!out)
			break;
		Log(Level::LowLevel, std::format(
			"fps flush {}x{} pts={}", out.Width(), out.Height(), out.Pts()));
		Save(std::move(out));
	}
	m_graph.reset();
}
