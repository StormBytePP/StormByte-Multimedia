	...
	for (const auto& av_stream : av_streams) {
		...
		switch(av_stream.Type()) {
			case AVMEDIA_TYPE_AUDIO: {
				...
				stream = std::make_shared<Stream>(*expected_codec, StormByte::Multimedia::Type::Audio);
			...
			case AVMEDIA_TYPE_VIDEO: {
				...
				stream = std::make_shared<Stream>(*expected_codec, StormByte::Multimedia::Type::Video);
			...
			case AVMEDIA_TYPE_SUBTITLE: {
				...
				stream = std::make_shared<Stream>(*expected_codec, StormByte::Multimedia::Type::Subtitle);
				// No additional context for subtitles currently
				break;
			}
			default: {
				// For streams that we do not modify like subtitles, data, attachments, etc.
				stream = std::make_shared<Stream>(*expected_codec, StormByte::Multimedia::Type::Unknown);
				break;
			}
		}
